        ...
        switch(key) {
            case "consolehelp":
                return  "Use one of the following commands to execute the specified action. \n"
                        + "More details on how to use this application can be found at help menu.\n"
                        + "----------------------------------------------------------------\n"
                        + "command\t\t\t action\n"
                        + "----------------------------------------------------------------\n"
            ...
            case "intro-html":             
                return "<html>"
                    + "<h1>Introduction</h1>"
                    + "<p>The aim of this application is to aid at understanding and analyzing the pi-calculus expressions by providing a possibility to view and interact with its graphical representation. It is, however, recommended that the users of this applications have at least basic theoretical knowledge of the pi-calculus.</p>"
                    + "<br>"                       
                    + "<p>The core functionallity of the application is described in the individual chapters of this help with refference to the following components provided by the application interface:</p>"
                    + "<ul>"
                    + "<li><b>MenuBar</b> comprised of 4 main menus - File, Control, Settings, Help</li>"
                    + "<li><b>ToolBar</b> for faster access to key items from menus</li>"
                    + "<li><b>Console</b> for text input and control (first tab on the top)</li>"
                    + "<li><b>Environment panel/editor</b> (second tab on the top)</li>"
            ...
            case "procspec-html":                
                return "<html>"
                    ...
                    + "<br>"
                    + "<p>Once the process is specified, its definition can be print out to the console using the command <b>env <FONT COLOR=RED>Id</FONT></b>. In case that the key word <b>env</b> is used withou process identifier, all process definitions are printed.</p>"
                    + "<br>"
                    + "<p>Another way how to add, modify or remove process definitions is to use the <b>Environment editor</b>. The syntax of process definition used in the editor is the same as in console. Remember that all changes made in the Environment editor needs to be commited in order to apply.</p>"
                    + "<br>"    
            ...
            case "procvis-html":                
                return "<html>"
                    ...
                    + "<br>"
                    + "<p>Another option how to visualize expression is via the dialog window which can be opened from Control menu by selecting item <b>Visualize expression</b> or using keyboard shortcut <b>F4</b>.</p>"
                    + "<br>"    
                    + "<p>In case that the specified expression contains concretization of process which was not defined in advance, the warning will be shown in the console. However, it is possible to define the process additionaly.</p>"
                    + "<br>"    
            ...
            case "reduction-html":                
                return "<html>"
                    + "<h1>Reduction</h1>"
                    + "<p>In order to execute reduction upon the visualized expression, one must first select appropriate nodes for reduction. This can be done eigher manually by selecting nodes in the graph as described in chapter 'Graph manipulation' or by selecting reduction from the Reduction list. Reduction list can be made visible or hidden from Settings menu (<b>Reduction list</b> item).<p>"
                    + "<br>"
            ...
            case "simplification-html":                
                return "<html>"
                    ...
                    + "<br>"
                    + "<p>The effect of simplification is the following: </p>"
                    + "<ul>"
            ...
            case "files-html":                
                return "<html>"
                    ...
                    + "<br>"
                    + "<p>The last (,but not least usefull) way how to generate output from the application is to export the graphical representation of the expression into one of the raster or vector formats offered by the <b>Export graph as</b> submenu in the File menu. The whole graph is always exported no matter if it is only partially visible on the canvas. The size of the exported graphic can be adjusted by zooming in or out with the mouse wheel.<p/>"
                    + "</html>"; 
            ...
            case "about-html":
                return "<html>"
                    ...
                    + "<br>"    
                    + "<p>It is available as an open source under Apache 2.0 license at:</p>"
                    + "<br>"
                    + "</html>";
